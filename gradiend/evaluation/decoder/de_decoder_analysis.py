...
class DeDecoderAnalysis(DecoderAnalysis):
    ...
    def prepare_texts(self, sentence, method='salazar'):
        ...
        encoded_new = self.tokenizer(sentence_split,padding=True, return_tensors="pt", return_offsets_mapping=True, is_split_into_words=True)
        ...
        for index, (token_ids, attention_mask) in enumerate(zip(token_idx, attentions_masks)):
        ...
        # basically do not mask any cls or whatever token or include in the calculation 
            ...
            effective_token_ids = [token_id for token_id in token_ids if token_id != self.pad_token_id and token_id != self.cls_token_id and token_id != self.sep_token_id]
...
# adjust for when the word is multiple tokens then -> have the mean of the tokens similar to sentence_scoring
    def per_token_score(self, input_ids, scores, word, reduction): 
        ...
        token_scores = {token_id.item(): score for token_id, score in zip(input_ids, scores)}


        word_token_ids = self.tokenizer(word)['input_ids']

        effective_word_token_ids = [token_id for token_id in word_token_ids if token_id != self.sep_token_id and token_id!= self.cls_token_id and token_id != self.pad_token_id]
    ...
    def evaluate_sentence(self, input, method, per_token = False, return_tensors = False, base_two = False, prob=False):
        ...
        if per_token: 
             return self.per_token_score(input_ids=original_ids, scores=scores, word=word, reduction=reduction)
    ...
    def evaluate_non_gender_mlm(self, max_size=10000):
        ...
        if is_generative:
            result = evaluate_clm_perplexity(model, self.tokenizer, texts[:1000], verbose=False)
            # todo gpt
            #if is_llama:
            #else:
    ...
    def evaluate_single_locus_grammaticality(self, df, top_k = None, batch_size=32):   
        ...
        df['masked_det'] =  df['masked_det'].apply(lambda x: self.preprocess(sentence=x, mask=mask))
        df['masked_ajd'] = df['masked_adj'].apply(lambda x: self.preprocess(sentence=x, mask=mask, det=False))
        ...
        output_data = {
            key: [] for key in ["acc_d"]
        }    


        output_data = {
                key: [] for key in 
                ["gram_det_p", "ungram_det_p", "gram_sent_prob", "ungram_sent_prob", 'gram_det_score', 'ungram_det_score', 'gram_adj_score', 'ugram_adj_score', 'most_likely_token', 'candidate_score_certainty','cf_candidate_score_certainty', 'candidate_det_certainty','cf_candidate_det_certainty', 'candidate_adj_certainty', 'cf_candidate_adj_certainty', 'gender']
        }
        for start_idx in range(0, len(masked_texts_det), batch_size):
            ...
            batch_tokenized_text = self.tokenizer(batch_texts_det, padding=True, return_tensors="pt", truncation=True)
            ...
            with torch.no_grad():
                output_det = self.model(input_ids=input_ids, attention_mask=attention_mask)
            ...
            for i in range(len(batch_texts_det)): 
                ...
                candidate_score_prob = self.evaluate_sentence(input= candidate_det, method='kauf_l2r', per_token=False, base_two=False, prob=True)[0][0]
                cf_candidate_score_prob = self.evaluate_sentence(input= cf_candidate_det, method='kauf_l2r', per_token=False, base_two=False, prob=True)[0][0]


                candidate_score_certainty = self.evaluate_sentence(input= candidate_det, method='kauf_l2r', per_token=False, base_two=False, prob=False)[0][0]
                cf_candidate_score_certainty =  self.evaluate_sentence(input= cf_candidate_det, method='kauf_l2r', per_token=False, base_two=False, prob=False)[0][0]


                # token scores for factual counterfactual DETERMINERS
                candidate_det_score = self.evaluate_sentence(input= candidate_det, method='kauf_l2r', per_token=True, base_two=False, prob=True)[1]
                cf_candidate_det_score = self.evaluate_sentence(input= cf_candidate_det, method='kauf_l2r', per_token=True, base_two=False, prob=True)[1]
                
                candidate_det_certainty = self.evaluate_sentence(input= candidate_det, method='kauf_l2r', per_token=True, base_two=False, prob=False)[1]
                cf_candidate_det_certainty = self.evaluate_sentence(input= cf_candidate_det, method='kauf_l2r', per_token=True, base_two=False, prob=False)[1]

             
                # token scores for factual counterfactual ADJECTIVES
                candidate_adj_score = self.evaluate_sentence(input= candidate_adj, method='kauf_l2r', per_token=True, base_two=False, prob=True)[1]
                cf_candidate_adj_score = self.evaluate_sentence(input= cf_candidate_adj, method='kauf_l2r', per_token=True, base_two=False, prob=True)[1]

                candidate_adj_certainty = self.evaluate_sentence(input= candidate_adj, method='kauf_l2r', per_token=True, base_two=False, prob=False)[1]
                cf_candidate_adj_certainty = self.evaluate_sentence(input= cf_candidate_adj, method='kauf_l2r', per_token=True, base_two=False, prob=False)[1]
                ...
                article_probs = {
                    ...
                    for art in ['Der', 'Des', 'Den', 'Dem', 'Die', 'Ein', 'Eines', 'Einen', 'Einem', 'Mein', 'Meines', 'Meinen', 'Meinem', 'Eine', 'Einer', 'Meine', 'Meiner']
                }

                sorted_probs = sorted(article_probs.items(), key=lambda x: x[1], reverse=True)
                ...
                output_data['candidate_score_certainty'].append(candidate_score_certainty)
                output_data['cf_candidate_score_certainty'].append(cf_candidate_score_certainty)
                output_data['candidate_det_certainty'].append(candidate_det_certainty)
                output_data['cf_candidate_det_certainty'].append(cf_candidate_det_certainty)
                output_data['candidate_adj_certainty'].append(candidate_adj_certainty)
                output_data['cf_candidate_adj_certainty'].append(cf_candidate_adj_certainty)
...
def evaluate_model_with_ae(model_path: str, eval_csv: str, output_csv: str, gender_factors=None, lrs=None):
        """
        ...
        if gender_factors is None:
            gender_factors = [-10, -2, -1, -0.8, -0.6, -0.4, -0.2,
                            0.0, 0.2, 0.4, 0.6, 0.8, 1, 2, 10]
        if lrs is None:
            lrs = [-1, -0.95, -0.9, -0.85, -0.8, -0.75, -0.7, -0.65,
                -0.6, -0.55, -0.5, 0, 0.5, 0.55, 0.6, 0.65, 0.7,
...
def compute_metrics(output_data):
    ...
    acc_sent_overall = (output_data['gram_sent_prob'] > output_data['ungram_sent_prob']).mean()
    acc_det_score_overall = (output_data['gram_det_score'] > output_data['ungram_det_score']).mean()
    acc_adj_score_overall = (output_data['gram_adj_score'] > output_data['ugram_adj_score']).mean()

    grouped_sent_acc = output_data.groupby('gender').apply(lambda x: (x['gram_sent_prob'] > x['ungram_sent_prob']).mean())
    grouped_det_acc = output_data.groupby('gender').apply(lambda x: (x['gram_det_score'] > x['ungram_det_score']).mean())
    grouped_adj_acc = output_data.groupby('gender').apply(lambda x: (x['gram_adj_score'] > x['ugram_adj_score']).mean())


    output_data['certainty_sent_overall'] = output_data.apply(lambda x: diff(x['candidate_score_certainty'],x['cf_candidate_score_certainty']), axis=1)
    output_data['certainty_det_overall'] = output_data.apply(lambda x: diff(x['candidate_det_certainty'],x['cf_candidate_det_certainty']), axis=1)
    output_data['certainty_adj_overall'] = output_data.apply(lambda x: diff(x['candidate_adj_certainty'],x['cf_candidate_adj_certainty']), axis=1)

    grouped_sent_certainty = output_data.groupby('gender')['certainty_sent_overall'].mean()
    grouped_det_certainty = output_data.groupby('gender')['certainty_det_overall'].mean()
    grouped_adj_certainty = output_data.groupby('gender')['certainty_adj_overall'].mean()
...
if __name__ == '__main__': 
    ...
    distilbert_model_path = "results/experiments/gradiend/MF/1e-05/bert-base-german-cased/0"
    pairs = {(gender_factor, lr) for gender_factor in default_evaluation_gender_factors for lr in default_evaluation_lrs}
    bert_with_ae = ModelWithGradiend.from_pretrained(distilbert_model_path)
