...
from gradiend.data.util import get_default_prediction_file_name, get_file_name, json_dumps, json_loads
...
class EncoderAnalysis(ABC):
    def __init__(self, config):
        ...
        self.articles = list({article for key in self.gender_keys for article in config['categories'][key]['articles']})
    ...
    @abstractmethod
    def get_model_metrics(self,*encoded_values, config, prefix=None, suffix='.csv', **kwargs):
        pass
    ...
    def read_default_predictions(self,model): 
        ...
        try:
            ...
            for i in self.articles: 
                cache_default_predictions[i] = cache_default_predictions[i].apply(json_loads)
    ...
    def write_default_predictions(self, default_predictions, model):
        ...
        for article in self.articles: 
            cache_default_predictions[article] = cache_default_predictions[article].apply(json_dumps)
    ...
    def plot_model_results(self,encoded_values, title='', y='z_score'):
        ...
        if y == 'encoded':
            plot_results.loc[plot_results['type'] == f"{self.det_combination} masked", 'plot_state'] = plot_results['state']
        else:
            plot_results = plot_results[plot_results['type'] == f"{self.det_combination} masked"].reset_index(drop=True)
    ...
    def get_std_stats(self, df):
        ...
        std_keys = {f"std_{key}": df[df['category'] == key]['encoded'].std() for key in self.gender_keys}
        std_diffs = {f"std_diff_{k1}_{k2}": abs(std_keys[k2] - std_keys[k1]) for k1, k2 in combinations(std_keys, 2)}
